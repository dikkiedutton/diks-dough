...
function loadSettings() {
  try {
    ...
    // Attempt to load independent hydration memory, or use old shared key as fallback
...
function saveSettings() {
  try {
    ...
    // Fail silently if browser blocks saving
  }
}
...
function resetSettings() {
  if (confirm("Are you sure you want to reset the calculator back to its default state? Your notes will not be deleted.")) {
    try {
      ...
      localStorage.removeItem('poolish_hydration'); // Clear specific memory
      localStorage.removeItem('sameday_hydration'); // Clear specific memory
      localStorage.removeItem('pizzaHydration');    // Clear old memory just in case
